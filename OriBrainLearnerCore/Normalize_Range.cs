...
namespace LibSVMScale
{
    ...
    public class Normalize_Range : NormalizationInterface
    {
        ...
        public void outputRealTime_update(int index, ref double value)
        {
           ...
           //    return;
            ...
            min = Preferences.Instance.TestingBaselineStatistics.getMedianWhenIGIndicesSaved(index) - Preferences.Instance.medianRange.feature_min[index];
            max = Preferences.Instance.TestingBaselineStatistics.getMedianWhenIGIndicesSaved(index) + Preferences.Instance.medianRange.feature_max[index];
            value = lower + (upper - lower) * (value - min) /
                    (max - min);
