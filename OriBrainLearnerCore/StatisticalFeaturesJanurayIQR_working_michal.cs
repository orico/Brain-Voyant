...
{ 
    ...
    /// </summary>
    public class StatisticalFeaturesJanurayIQR_working_michal
    {
        public double[] mean;
        public double[] std;
        ...
        int skipFirstTrs = 2; //was zero 17\2\2014
        ...
        { 
            ...
            if (currentTR < (int)GuiPreferences.Instance.NudMovingWindow) //we calculate using first k-2 baseline until we accumulate K samples
            {
                //NOTE: expanding window will let the window grow from the amount of baseline trs until the size of the actual window, 10->50 .                
                int window = Preferences.Instance.events.eventList[0].var2;
                if (ExpandingWindow)
            ...
            else
            {
                baselineArray = new double[(int)GuiPreferences.Instance.NudMovingWindow];
                int index = 0;
                for (int i = currentTR - (int)GuiPreferences.Instance.NudMovingWindow; i < currentTR; i++)
                {
                    baselineArray[index] = matrix[feature][i];  //matrix is also 0-based
                    index++;
                }
        ...
        public StatisticalFeaturesJanurayIQR_working_michal(int _features, int _vectors, int[] _trainTopIGFeatures, bool _isBaseline)
        {
