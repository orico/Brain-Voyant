...
{
    public class Normalize_MinMax : NormalizationInterface
    {
        ...
        public void outputRealTime_update(int index, ref double value)
        {
            ...
            if (feature_max[index] == feature_min[index])
                return;
           // if (Preferences.Instance.currentClassifiedVector <= 128)
            { 
                if (value < feature_min[index])
                {
                    feature_min[index] = value;
                }

                if (value > feature_max[index])
                {
                    feature_max[index] = value;
                }
            }   
        ...
        /// <summary>
        /// function, returns max index, min values, max values arrays as a reference
        /// </summary>
        ...
        /// <summary>
        /// was static, can be static.
        /// </summary>
        /// <param name="save_filename"></param>
        /// <param name="feature_min"></param>
        /// <param name="feature_max"></param>
        ...
        {
            Normalize_MinMax s = new Normalize_MinMax();
            s.run(argv);
        }
    }
}
